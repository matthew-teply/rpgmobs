...
public class ConversationMenu extends ModularUIContainerMenu {
    public ConversationMenu(int windowID, Inventory inventory, AbstractNPC<?> entity) {
        super(ModMenuTypes.CONVERSATION_MENU.get(), windowID, inventory, entity);
    }

    private static SimpleMenuProvider getMenuProvider(AbstractNPC<?> entity) {
     ...
     */
    public static void open(ServerPlayer player, AbstractNPC<?> entity) {
        player.openMenu(
    ...
    public static MenuType<ModularUIContainerMenu> getMenuType() {
        return IMenuTypeExtension.create((windowId, inventory, data) -> {
            ...
            if (entity instanceof AbstractNPC<?> humanlikeEntity) {
